...
async def start(client, message):
            ...
            if message.command[1] != "subscribe":
                if REQUEST_TO_JOIN_MODE == True:
                    ...
                    try:
                        ...
                        btn.append([InlineKeyboardButton("↻ ᴠᴇʀɪғɪᴇᴅ ᴀɢᴀɪɴ", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
            if REQUEST_TO_JOIN_MODE == True:
                if TRY_AGAIN_BTN == True:
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            filesarr.append(msg)
        k = await client.send_message(chat_id = message.from_user.id, text=f"تنويه سيتم الحذف بعد 10 دقايق لهيك بعتو ع الرسائل المحفوظة او اي مكان منشان فيك ضل تشوفو 🫥")
        await asyncio.sleep(600)
        for x in filesarr:
            await x.delete()
        await k.edit_text("<b>✅ تم حذف الملف بنجاح لتواصل @kinanalyousef</b>")
        return    
